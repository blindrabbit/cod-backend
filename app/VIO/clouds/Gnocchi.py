...
from gnocchiclient.exceptions import ArchivePolicyNotFound, MetricNotFound, NamedMetricAlreadyExists, \
    ResourceTypeAlreadyExists, ResourceTypeNotFound
...
class Gnocchi:
    ...
    def set_create_resource_type(self, name):
        try:
            self.resource_type = {"name": name, 'attributes': {
                "host": {"type": "string", "min_length": 0, "max_length": 255, "required": True}}}
    ...
    def set_create_archive_policy(self, name):
        try:
            self.create_metric = self.gnocchi_client.archive_policy.create({'name': name, 'back_window': 0,
                                                                            'definition': [
    ...
    def get_last_measure(self, name_metric, resource_id, aggregation, granularity, start, stop):
        ...
        dados = self.gnocchi_client.metric.get_measures(metric=name_metric,
                                                        start=start,
                                                        stop=stop,
    ...
    def get_last_measure_Date(self, name_metric, resource_id, aggregation, granularity, start, stop):
        try:
            dados = self.gnocchi_client.metric.get_measures(name_metric, start, stop, aggregation, granularity,
                                                            resource_id)
    ...
    # If dont data, return -1, else return data
    def get_measure_in_interval(self, name_metric, resource_id, aggregation, granularity, start, stop):
        dados = self.gnocchi_client.metric.get_measures(metric=name_metric,
                                                        start=start,
                                                        stop=stop,
                                                        aggregation=aggregation,
                                                        granularity=granularity,
                                                        resource_id=resource_id)
        df = pd.DataFrame(dados, columns=['timestamp', 'granularity', 'metric_utilization'])
        if df.__len__() == 0:
            return -1
